...
object V1Routes {
  def routes(authService: AuthServiceInterface)(implicit authUserRepository: AuthUserRepository): HttpRoutes[IO] =
    HealthRoutes.routes  <+> new AuthRoutes(authService, new AuthJWTMiddleware()).routes
}
